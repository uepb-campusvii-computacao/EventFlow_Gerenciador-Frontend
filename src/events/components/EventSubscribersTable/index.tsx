...
interface EventSubscribersTableState {
...
export function EventSubscribersTable({ data }: EventSubscribersTableProps) {
  const [state, setState] = useState<EventSubscribersTableState>({
    users: data,
    currentPage: 1,
    usersPerPage: 20,
    filter: '',
        ...
        >
          <div className='flex flex-col gap-2 text-black'>
            ...
            <label
              htmlFor='pedente'
              className={`rounded-md bg-gray-200 px-3 py-2 text-sm transition-colors hover:bg-gray-400 ${
                state.filter === 'PENDENTE' && 'bg-gray-400'
              }`}
            >
              <input
                onChange={e => toggleFilter(e.target.value)}
                value='PENDENTE'
                checked={state.filter === 'pedente'}
                className='hidden'
                type='checkbox'
                name=''
                id='pedente'
